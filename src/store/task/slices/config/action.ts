...
export class TaskConfigSliceActionImpl {
  ...
  // TODO [LOBE-6587]: Arvin 完成周期/定时执行 infra 后，在此新增：
  // updateHeartbeatInterval(id, interval) — 配置周期执行间隔（heartbeatInterval 字段）
  // updateSchedule(id, { pattern, timezone }) — 配置 cron 定时任务（schedulePattern + scheduleTimezone 字段）
  // 说明：heartbeatInterval 控制周期执行，heartbeatTimeout+lastHeartbeatAt 用于 watchdog 监控，是两个不同用途
}
